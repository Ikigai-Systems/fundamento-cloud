...
RSpec.describe PolicyUserContext, type: :model do
  ...
  describe "#initialize with OrganizationMembership" do
    subject { described_class.new(om_is_pawel) }

    it "sets @user to organization_membership's user" do
      expect(subject.user).to eq(pawel)
    end

    it "sets @current_organization to organization_membership's organization" do
      expect(subject.current_organization).to eq(is_org)
    end

    it "sets @organization_membership to the passed organization_membership" do
      expect(subject.organization_membership).to eq(om_is_pawel)
    end
    ...
    it "exposes organization_membership via attr_reader" do
      expect(subject).to respond_to(:organization_membership)
      expect(subject.organization_membership).to be_a(OrganizationMembership)
    end
  end
  ...
  describe "#initialize with User and Organization" do
    ...
    it "looks up @organization_membership by user and organization" do
      expect(subject.organization_membership).to eq(om_is_pawel)
      expect(subject.organization_membership).to be_a(OrganizationMembership)
    end

    context "when user belongs to organization" do
      it "finds the correct organization_membership" do
        context = described_class.new(stefan, is_org)

        expect(context.organization_membership).to eq(om_is_stefan)
      end

      it "finds organization_membership with correct role" do
        context = described_class.new(pawel, is_org)

        expect(context.organization_membership.role).to eq("manager")
      end
    end

    context "when user belongs to multiple organizations" do
      it "finds organization_membership for the specified organization" do
        # Pawel is in both is_org and hc_org
        context_is = described_class.new(pawel, is_org)
        context_hc = described_class.new(pawel, hc_org)

        expect(context_is.organization_membership).to eq(om_is_pawel)
        expect(context_is.organization_membership.organization).to eq(is_org)

        expect(context_hc.organization_membership).to eq(om_hc_pawel)
        expect(context_hc.organization_membership.organization).to eq(hc_org)
      end
    end
    ...
    context "when user does not belong to organization" do
      ...
      it "sets @organization_membership to nil" do
        expect(subject.organization_membership).to be_nil
      end
  ...
  describe "type detection" do
    it "detects OrganizationMembership input correctly" do
      context = described_class.new(om_is_pawel)

      expect(context.organization_membership).to eq(om_is_pawel)
    ...
    it "differentiates between OrganizationMembership and User" do
      ...
      expect(ou_context.organization_membership).to eq(user_context.organization_membership)
    end
  end
  ...
  describe "use in Pundit policies" do
    ...
    it "provides organization_membership for policy checks" do
      context = described_class.new(pawel, is_org)

      expect(context.organization_membership).to be_present
      expect(context.organization_membership.manager?).to be true
    end
    ...
    it "allows policy to check user role" do
      ...
      expect(manager_context.organization_membership.manager?).to be true
      expect(member_context.organization_membership.member?).to be true
    end
  end
  ...
  describe "edge cases" do
    it "handles user with no organization memberships" do
      ...
      expect(context.current_organization).to eq(is_org)
      expect(context.organization_membership).to be_nil
    end
    ...
    it "returns consistent results when called multiple times" do
      ...
      expect(context1.organization_membership).to eq(context2.organization_membership)
    end
  end
  ...
  describe "immutability" do
    ...
    it "exposes attributes as read-only" do
      ...
      expect(subject).not_to respond_to(:organization_membership=)
    end
  end
end
