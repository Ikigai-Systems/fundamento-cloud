...
RSpec.describe Formula::ActionTracker, type: :model do
  ...
  describe '#record_action' do
    it 'records actions with their parameters' do
      action_tracker.record_action("AddRow", tableNpi: "table1", values: { "col1" => "val1" })
  ...
  describe 'action functions integration' do
    ...
    describe 'RunActions' do
      it 'executes nested actions' do
        ...
        expect(result).to eq(true)
        expect(action_tracker.get_actions).to include(
        ...
        result2 = engine.evaluate(formula2, action_tracker:)
  ...
  describe '#clear' do
    it 'clears all recorded actions' do
      action_tracker.record_action("AddRow", tableNpi: "table1")
      expect(action_tracker.has_actions?).to eq(true)
      
      action_tracker.clear
      expect(action_tracker.has_actions?).to eq(false)
      expect(action_tracker.get_actions).to eq([])
    end
  end
end