...
const createFlash = (options: FlashOptions) => {
  ...
  <div data-flash-key="${options.key}"
      ...
      class="w-full flex items-end justify-right nth-[n+2]:mt-2 first:mt-10 justify-end z-30 pointer-events-none relative left-96">
    <div class="max-w-sm w-full relative pointer-events-auto bg-${color}-100 dark:bg-${color}-800 border-t-4 border-${color}-500 dark:border-${color}-500 rounded-b text-${color}-900 dark:text-${color}-50 px-4 py-3 shadow-md">
      <div class="flex items-start">
        ...
        <div class="ml-4 shrink-0 flex">
          <button data-action="alert#close" class="inline-flex text-${color}-500 dark:text-${color}-50 focus:outline-hidden focus:text-gray-300 transition ease-in-out duration-150">
