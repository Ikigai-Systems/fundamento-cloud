...
section :table_helpers do
  ...
  def tables.create_from_definition(label = nil, definition_path:, space:, organization:, **attrs)
    ...
    # A single bulk_imported change event for the whole seeded table, rather than one
    # per cell: seeding a scenario would otherwise write hundreds of thousands of them.
    Tables::ChangeRecorder.bulk(table, kind: :bulk_imported, payload: summary) do
      ...
      summary[:rows] = table.rows.count
      summary[:columns] = table.columns.count
    end
